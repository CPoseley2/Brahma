...
async function renderSession(snapshot = services.auth.snapshot) {
  ...
  try {
    ...
    const viewModel = new AppViewModel(model, identity, { media: import.meta.env.VITE_FIREBASE_STORAGE_ENABLED === "true" ? services.media : null, teamId: services.repository.teamId });
    ...
    new ShellView(root, viewModel, views, services.auth).mount();
  } catch (error) {
